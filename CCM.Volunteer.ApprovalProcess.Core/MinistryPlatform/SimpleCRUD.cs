...
namespace Dapper
{
    ...
    public static class SimpleCRUD
    {
        ...
        /// <param name="id"></param>
        /// <returns>Returns a single entity by a single id from table T.</returns>
        public static T GetWithImageInfo<T>(this IDbConnection connection, object id)
            where T : BaseEntity
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            ...
            var sb = new StringBuilder();
            sb.AppendLine(string.Format("DECLARE @PageID int = (SELECT Page_ID FROM dp_Pages WHERE Display_Name='{0}')", name.Remove(0,1).Remove(name.Length - 2, 1)));
            sb.AppendLine(string.Format("Select * from {0} as x", name));
            sb.AppendLine(string.Format("LEFT JOIN dp_Files as cFiles ON cFiles.Record_ID = x.{0}  AND cFiles.Page_ID = @PageID AND cFiles.Default_Image = 1", onlyKey.Name));
            sb.Append(" where " + onlyKey.Name + " = @Id");
            ...
            return connection.Query<T, dpFile, T>(
                sb.ToString(),
                (obj, file) => { 
                    obj.DefaultImage = file;
                    if (file != null)
                        obj.DefaultImageUrl = String.Format(Constants.GeneralImageBaseAppUrl, file.Table_Name.ToLower(), id, file.File_ID);
                    else
                        obj.DefaultImageUrl = String.Format(Constants.GeneralImageBaseAppUrl, "contacts", id, 0);
                    return obj; }, splitOn: "File_ID", 
                    param: dynParms).FirstOrDefault();
        }
        ...
        /// <param name="whereConditions"></param>
        /// <returns>Gets a list of entities with optional exact match where conditions</returns>
        public static IEnumerable<T> GetListWithImageInfo<T>(this IDbConnection connection, object whereConditions = null)
            where T : BaseEntity
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            ...
            if (whereConditions == null)
                whereConditions = new object { };

            var whereprops = GetAllProperties(whereConditions).ToArray();
            sb.AppendLine(string.Format("DECLARE @PageID int = (SELECT Page_ID FROM dp_Pages WHERE Display_Name='{0}')", name.Remove(0, 1).Remove(name.Length - 2, 1)));
            sb.AppendLine(string.Format("Select * from {0} as x", name));
            sb.AppendLine(string.Format("LEFT JOIN dp_Files as cFiles ON cFiles.Record_ID = x.{0}  AND cFiles.Page_ID = @PageID AND cFiles.Default_Image = 1", idProps.First().Name));
            ...
            .Query<T, dpFile, T>(
                ...
                (obj, file) => { 
                    obj.DefaultImage = file;
                    if (file != null)
                        obj.DefaultImageUrl = String.Format(Constants.GeneralImageBaseAppUrl, file.Table_Name.ToLower(), idProps.First().GetValue(obj), file.File_ID);
                    else
                        obj.DefaultImageUrl = String.Format(Constants.GeneralImageBaseAppUrl, "contacts", idProps.First().GetValue(obj), 0);
                    return obj; }, splitOn: "File_ID", 
